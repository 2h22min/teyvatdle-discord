...
class Character:
    attributes = ["name", "nation", "height", "vision", "weapon", "version"]
    ...
    def compareTo(self, guess):
        common = {key: 1 for key in __class__.attributes}
        ...
        for attr in common:
            ...
            if attr == "height" or attr == "version":
                ...
                c["img"] = Image.new("RGBA", size, background)
                foreground = ImageDraw.Draw(c["img"])
                foreground.text((size[0]/2,size[1]/2), guess_dict[attr], "white", anchor="mm", stroke_width=strokewidth, font_size=textsize)
            else:
        ...
        texts.text((fullsize[0]//2, fullsize[1]//4), guess.name, fill=(240, 220, 200), anchor="mm", stroke_width=20, stroke_fill=(96, 64, 48), font_size=150)
        texts.rectangle([(c_x,title_y-c_sep*1.25),((len(cards)*(size[0]+c_sep))-c_sep-1,title_y+c_sep*1.25)], "white")

        for c in cards:
            if c != "name":
                texts.text(((c_x + size[0]//2), title_y), c, fill=(96, 64, 48), anchor="mm", stroke_width=2, stroke_fill=(96, 64, 48), font_size=34)
...
def getCharacters(**conditions):
    try:
        ...
        for key in conditions:
            if key in Character.attributes:
                keys += f" AND {key} = ?"
        ...
        return [Character(*attrs) for attrs in c.fetchall()]
    finally:    
        c.close()
        con.close()
...
if __name__ == "__main__":
    ...
    # print(char, guess, char.compareTo(guess), sep="\n")
    ...
    print(Character.random().compareTo(Character.random()))
